...
int force_treeevaluate(int target, int mode, double *ewaldcountsum)
{
...
#ifdef FDM
  double fdm = 0.0;
  double fdm_acc_x, fdm_acc_y, fdm_acc_z;
  fdm_acc_x = 0;
  fdm_acc_y = 0;
  fdm_acc_z = 0;
  double lambda = All.FdmKernelLambda;
  double norm = All.FdmNormMass;
  double axion = All.FdmAxionMass;
#ifdef FDMADAPTIVE
  double v = 1.0;
  ...
  if(lambda<All.FdmKernelLambda)
  {
	  lambda = All.FdmKernelLambda;
  }
#endif
#ifdef FDMTEST
  printf("target = %d\n",target);
...
#ifdef FDMBIAS
  double len1, len2, num1, num2, D, bias;
  no = Father[target];
  if(no<All.MaxPart + MaxNodes && no>=All.MaxPart)
  {
	nop = &Nodes[Father[target]];
	len1 = nop->len;
	num1 = nop->u.d.mass/P[target].Mass;
  }
  else
  {
	 //printf("father is %d\n",no);
...
#ifdef FDMBIASTEST
  if(nop->u.d.mass>=0.00001)
  {
    printf("nop = %d\n",nop);
    printf("nop len = %f, nop mass = %f",nop->len,nop->u.d.mass);
    getchar();
  }
#endif
  nop = 0;  
...
#ifdef FDMTEST
      printf("beforeFDM,r=%lf,mass=%lf,ratio=%f,fac=%lf\n",r,mass,mass/P[no].Mass,fac);
      getchar();
#endif
#ifdef FDM
...
#ifdef FDMBIAS
        len2 = nop->len;
		num2 = nop->u.d.mass/P[target].Mass;
...
#ifdef FDM
  P[target].QuanAccel[0] = fdm_acc_x;
  P[target].QuanAccel[1] = fdm_acc_y;
  P[target].QuanAccel[2] = fdm_acc_z;
#endif
#ifdef PERIODIC
...
#ifdef FDMTEST
      printf("short,afterFDM,after short table,r=%lf,mass=%lf,ratio=%f,fac=%lf\n",r,mass,mass/P[no].Mass,fac);
      printf("node information, len=%f, mass=%f, father=%d\n",nop->len,nop->u.d.mass,nop->u.d.father);
      getchar();
#endif
