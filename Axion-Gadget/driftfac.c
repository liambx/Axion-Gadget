...
#ifdef FDM
	  F.function = &quankick_integ;
      gsl_integration_qag(&F, exp(logTimeBegin), exp(logTimeBegin + ((logTimeMax - logTimeBegin) / DRIFT_TABLE_LENGTH) * (i + 1)), 0,
			  1.0e-8, WORKSIZE, GSL_INTEG_GAUSS41, workspace, &result, &abserr);
      QuanKickTable[i] = result;
#endif
    }
 ...
 *  the gravitational force.
 */
double get_quankick_factor(int time0, int time1)
{
  double a1, a2, df1, df2, u1, u2;
  int i1, i2;
  ...
  if(i1 <= 1)
    df1 = u1 * QuanKickTable[0];
  else
    df1 = QuanKickTable[i1 - 1] + (QuanKickTable[i1] - QuanKickTable[i1 - 1]) * (u1 - i1);
  ...
  if(i2 <= 1)
    df2 = u2 * QuanKickTable[0];
  else
    df2 = QuanKickTable[i2 - 1] + (QuanKickTable[i2] - QuanKickTable[i2 - 1]) * (u2 - i2);
...
/*! Integration kernel for quantum pressure kick factor computation.
 */
double quankick_integ(double a, void *param)
{
  double h;
